...
class ConfigEntry {
  const ConfigEntry({required this.section, required this.key, required this.value});
...
class SpicetifyConfig {
  ...
  static String normalizeSection(String section) => _aliases[section] ?? section;

  String? value(String section, String key) {
    for (final entry in entries) {
      if (normalizeSection(entry.section) == normalizeSection(section) && entry.key == key) {
        return entry.value;
      }
    }
    return null;
